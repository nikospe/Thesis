...
function storesLoad() {
    if ( window.location.pathname.includes('product') ) {        
        ...
        var mdata = { 'id' : serviceId, 'address': decodeURIComponent(servAddress).trim() };
...
function storeAppend (store) {
    ...
    <div class='row marg-left'>
        <div class='col-md-8 descript-stores white-background stores stores-left left-right-shadow'>
            <div class='row'>
                <hr>
                <div class='col-md-3 visible-lg-block shop-logo-row'>
                    <a class='shop-logo' href='javascript:;' onClick='test(${store.id}, "${address}");'>
                        <img class='stores-image' alt='' data-src='' src='icons/map.png'></a>
                    <br><hr class='fif'>
                    <div class='changeme' id='r${store.id}'></div>
                    <span>
                </div>
                <p id='r"+store.id+"'></p>
                <div class='col-md-9'>
                    <a href='javascript:;' onClick='test(${store.id}, "${address}");'>
                        <h3 class='titles'>${store.name}</h3></a>
                    <h4 class='addresses'>${store.address}</h4>
                    <h5>Click the store icon to find it on Google map and rate store.</h5>                
                </div>
            </div>
            <div class='row google_map_none ${+store.id}'>
                <div class='col-md-12'>
                    <div class='marg' id='${store.id}' style='width:100%;height:350px'></div>
                    <hr><h4 class='submain-color'>Rate this store:</h4>
                    <form class='store-rating' id='${store.id}'>
                        ...
                        <p class='leave-blank' style='display:none;'>Title and rating must not be empty!</p>
                    </form>
                </div>
                <div class='col-md-12 marg'>
                    <a href='javascript:;' onClick='showRatings(${store.id})'>See ratings
                        <span class='glyphicon glyphicon-chevron-right main-blue'></span>
                    </a>
                    <div class='marg stor_com${store.id}' style='display:none;' id='store-comments'>
                        <div id='c${store.id}'></div>
                            <h4 class='submain-color'>Ratings: </h4>
                            <hr>
                            <div id='rr${store.id}'></div>
                        </div>
                    </div>
                </div>
            </div>
...
window.onload = function () {
    if ( locations.length == 0 ){
        $('#all_stores_map').removeClass('shadow-3');
    }
    if (screen.width < 960) {
        $('.caption').remove();
        $('.marg-left').removeClass('marg-left');
        $('.stores-service-left').removeClass('stores-service-left');
        $('.stores').removeClass('stores-left');
    }
};